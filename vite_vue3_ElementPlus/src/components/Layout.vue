<template>
  <el-container>
    <el-header>
      <el-menu :default-active="activeIndex" style="justify-content: space-between;" mode="horizontal" :router="true">
        <div style="display: flex;">
          <el-sub-menu index="brand" :show-timeout="0">
            <template #title>
              <span v-text="brand" style="font-size: 20px;vertical-align:baseline;"></span>
            </template>
        ...
        <div>
          <el-menu-item
            v-if="currentUser.groupId<='2'"
            style="float: right"
            @click="openOptionSettingDialog"
          >
            <el-icon>
              <el-icon-Setting />
            </el-icon>选项设置
          ...
          <el-sub-menu
            ...
            :show-timeout="0"
          >
            <template #title>Excel 操作</template>
            <el-menu-item @click="importFromExcelDialog.visible=true">
              <el-icon>
                <el-icon-UploadFilled />
              </el-icon>导入数据 从Excel
            </el-menu-item>
            <el-menu-item @click="openExportToExcelDialog">
              <el-icon>
                <el-icon-Download />
              </el-icon>导出数据 为Excel
            </el-menu-item>
    ...
    <el-dialog
      title="导出数据为 Excel"
      v-model="exportToExcelDialog.visible"
    ...
    >
      <el-form>
        <el-form-item>
          <el-select v-model="exportToExcelDialog.accountName" placeholder="请选择要导出的账户">
            <el-option
              v-for="account in exportToExcelDialog.accountList"
              :key="account.id"
              :label="account.name"
              :value="account.name"
            ></el-option>
      ...
      <template #footer>
        <div class="dialog-footer" v-show="exportToExcelDialog.accountName">
    ...
    <el-dialog
      title="导入数据从 Excel"
      v-model="importFromExcelDialog.visible"
      @closed="onImportExcelDialogClosed"
    >
      ...
      <upload
        ref="uploadExcel"
        :url="exportToExcelDialog.downloadUrl"
        :limit="1"
        :multiple="false"
        accept=".xlsx"
        :limit-size="1024"
    ...
    <el-dialog
      title="选项设置"
      v-model="optionSettingDialog.visible"
        ...
        <template #header>
          <div>
            <el-input
              ...
              @input="$refs.tree.filter(optionSettingDialog.filterText);"
              v-model="optionSettingDialog.filterText"
              clearable
            ></el-input>
          </div>
        </template>
          ...
          <template #default="{ node, data }">
            <span class="custom-tree-node">
              ...
              <span>
                <el-link
                  type="success"
                  @click="addOption(data,node)"
                  v-if="node.level===1"
                  :underline="false"
                >添加</el-link>
                <el-link
                  type="warning"
                  @click="updateOption(data,node)"
                  v-if="node.level===2"
                  :underline="false"
                >修改</el-link>
              </span>
...
export default {
  ...
  data() {
    return {
      ...
      exportToExcelDialog: {
        ...
        accountName: "",
        accountList: [],
        downloadUrl: `${import.meta.env.VITE_BaseURL}/excel/`
      }
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      window.open(
        exportToExcelDialog.downloadUrl +
          "?accountName=" +
          encodeURIComponent(exportToExcelDialog.accountName)
      );
