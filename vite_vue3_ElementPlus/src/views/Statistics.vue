    ...
    >
      <el-tab-pane label="表格数据">
        <el-table
          ...
          height="340px"
        >
          <template slot="empty" v-if="typeof queryCondition.accountId ==='string'">
            <span style="font-size: 18px" class="el-icon-top">
              请先选择
              <strong>银行账户</strong>
            </span>
          </template>
      ...
      <el-tab-pane
        label="收支变化图表"
        name="moneyChangedChart"
        :disabled="typeof queryCondition.accountId==='string'"
      >
        <div ref="moneyChangedChart" style="width: 100%;height:62vh"></div>
      </el-tab-pane>
      <el-tab-pane
        label="类别支出占比图表"
        name="categoryProportionChart"
        :disabled="typeof queryCondition.accountId==='string'"
      >
        <div ref="categoryProportionChart" style="width: 100%;height:62vh"></div>
      </el-tab-pane>
      <el-tab-pane
        label="部门支出占比图表"
        ...
        :disabled="typeof queryCondition.accountId==='string'"
      >
        <div ref="departmentProportionChart" style="width: 100%;height:62vh"></div>
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onTabClick(instance) {
      switch (instance.props.name) {
        case "moneyChangedChart":
          if (!this.echartsData.moneyChangedChart.instance) {
            setTimeout(() => {
              this.echartsData.moneyChangedChart.instance = echarts.init(
                this.$refs.moneyChangedChart,
                null,
                { renderer: "canvas" }
              );
        ...
        case "categoryProportionChart":
          if (!this.echartsData.categoryProportionChart.instance) {
            setTimeout(() => {
              this.echartsData.categoryProportionChart.instance = echarts.init(
                this.$refs.categoryProportionChart,
                null,
                { renderer: "canvas" }
              );
              this.echartsData.categoryProportionChart.instance.setOption(
                this.echartsData.categoryProportionChart.option,
                true
              );
            }, 200);
          }
        ...
        case "departmentProportionChart":
          if (!this.echartsData.departmentProportionChart.instance) {
            setTimeout(() => {
              this.echartsData.departmentProportionChart.instance = echarts.init(
                this.$refs.departmentProportionChart,
                null,
                { renderer: "canvas" }
              );
              this.echartsData.departmentProportionChart.instance.setOption(
                this.echartsData.departmentProportionChart.option,
                true
              );
            }, 200);
          }
      ...
      switch (chartInstance) {
        ...
        case this.echartsData.moneyChangedChart.instance:
          ...
          if (this.echartsData.moneyChangedChart.instance) {
            ...
            this.echartsData.moneyChangedChart.instance.setOption(
              this.echartsData.moneyChangedChart.option,
              true
            );
          }
    ...
    handleDepartmentProportionChartOption() {
      ...
      return statisticsApi
        .getDepartmentProportionData(queryCondition)
        .then(result => {
    ...
    handleCategoryProportionChartOption() {
      ...
      return statisticsApi
        .getCategoryProportionData(queryCondition)
        .then(result => {
          const data = result.data;
          const names = [];
