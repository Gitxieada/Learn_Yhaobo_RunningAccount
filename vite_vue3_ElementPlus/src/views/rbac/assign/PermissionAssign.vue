        ...
        <template #default="{ node, data }">
          <span class="custom-tree-node">
            ...
            <span>
              <el-link
                type="primary"
                @click.stop="assignPermission(data,node)"
                v-if="node.level===2"
                :underline="false"
              >分配权限</el-link>
            </span>
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button
            size="medium"
            type="primary"
            @click="submitAssignPermission()"
            :loading="assignPermissionDialog.loading"
          >确 定</el-button>
        </div>
      </template>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    submitAssignPermission() {
      ...
      roleApi
        .assignPermissions(
          this.assignPermissionDialog.assignPermissionIds,
          this.assignPermissionDialog.roleId
        )
        .then(() => {
