        ...
        <template #default="{ node, data }">
          <span class="custom-tree-node">
            ...
            <span>
              <el-link
                type="success"
                @click.stop="addUser(data,node)"
                v-if="node.level===2"
                :underline="false"
              >新增用户</el-link>
              <el-link
                type="warning"
                @click.stop="alterUser(data,node)"
                v-if="node.level===3"
                :underline="false"
              >修改此用户</el-link>
              <el-link
                type="danger"
                @click.stop="deleteUser(data,node)"
                v-if="node.level===3&&data.id!=='1'"
                :underline="false"
              >删除此用户</el-link>
            </span>
          </span>
...
export default {
    ...
    return {
      ...
      addUserDialog: {
        ...
        rules: {
          ...
          checkPassword: [{ validator: this.validateInput, trigger: "blur" }]
        },
        visible: false
      },
      addGroupDialog: {
  ...
  methods: {
    handleTreeNodeDrop(draggingNode, dropNode) {
      ...
      userApi
        .update({ id: userId, groupId: groupId })
        .then(() => {
          this.$message.success({
            message: `成功将【${draggingNode.data.name}】加入【${dropNode.data.name}】`,
            showClose: true
          });
        })
        .catch(() => {
      ...
      switch (rule.field) {
        ...
        case "password":
          if (!value) {
            ...
            callback(
              new Error("密码只允许[字母,数字,下划线,空格],且不超过32位")
            );
          } else {
            callback();
          }
        ...
        case "checkPassword":
          if (!value) {
            ...
            !(
              value === this.addUserDialog.user.password ||
              value === this.alterUserDialog.user.password
            )
          ) {
    ...
    loadTreeData() {
      groupApi.listGroupAndUser().then(result => {
        ...
        console.log("tree.data", toRaw(this.tree.data));
      });
    },
