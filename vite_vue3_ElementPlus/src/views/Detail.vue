<template>
  <div>
    <el-row type="flex" align="middle" :gutter="10">
      ...
      <el-col :span="3">
        <el-select
          v-model="queryCondition.accountId"
          placeholder="所有银行账户"
          @change="loadData"
          clearable
        >
      ...
      <el-col :span="3">
        <el-select
          v-model="queryCondition.departmentId"
          placeholder="所有部门"
          @change="loadData"
          clearable
        >
      ...
      <el-col :span="3">
        <el-select
          v-model="queryCondition.categoryId"
          placeholder="所有类别"
          @change="loadData"
          clearable
        >
    ...
    >
      <el-table-column type="expand">
        <template #default="scope">
          ...
          <el-collapse @change="showPicture($event,scope.row.id,scope.$index)" value="picture">
            <el-collapse-item
              :name="scope.row.pictures?'picture':''"
              :disabled="!scope.row.hasPicture"
            >
              <template #title>
                <span v-show="!scope.row.hasPicture">暂无图片信息</span>
                <span v-show="scope.row.hasPicture">图片信息(点击箭头展开)</span>
                <el-button
                  style="margin-left: 10px"
                  size="mini"
                  type="success"
                  v-if="currentUserGroupId<='1'"
                  @click.stop="openAddPictureDialog(scope.row.id,scope.$index,true)"
                >上传</el-button>
              </template>
              <div
                :element-loading-text="loadingText"
                v-loading="pictureCollapse.loading"
                style="height:inherit"
              >
                <el-tag
                  v-for="picture of scope.row.pictures"
                  :key="picture.id"
                  type="danger"
                  effect="plain"
                  :disable-transitions="true"
                  style="height: max-content;margin: 5px;"
                  closable
                ...
                >
                  <el-image
                    ...
                    :max-scale="7"
                    :min-scale="0.2"
                    :preview-src-list="previewSrcListFilter(scope.row.pictures)"
                    :src="picture.url"
                  ></el-image>
                </el-tag>
              </div>
      ...
      <el-table-column label="收入" prop="earning" :sortable="true">
        <template #default="scope">{{ '￥' + scope.row.earning }}</template>
      </el-table-column>
      <el-table-column label="支出" prop="expense" :sortable="true">
        <template #default="scope">{{ '￥' + scope.row.expense }}</template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="157" v-if="currentUserGroupId<='1'">
        <template #header>
          操作
          <el-button
            v-show="tableData.multipleSelectionData.data.length===0"
            size="small"
            type="success"
            style="margin-left: 10px"
          ...
          <el-button
            ...
            type="primary"
            style="margin-left: 0"
            @click="generateExpenseClaimForm"
            :loading="tableData.multipleSelectionData.loading"
          >生成报销凭证</el-button>
        </template>
        <template #default="scope">
          <el-button size="mini" @click.stop="openUpdateDetailDialog(scope.$index, scope.row)">修改</el-button>
          <el-button
            size="mini"
            type="danger"
            @click.stop="deleteDetail(scope.$index, scope.row)"
          >删除</el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog
      title="修改记录"
      v-model="updateDetailDialog.visible"
      width="30%"
      :close-on-click-modal="false"
    >
      <el-form
        label-width="70px"
        :model="updateDetailDialog.detail"
      ...
      >
        ...
        <el-form-item label="收入" prop="earning">
          <el-input-number
            v-model="updateDetailDialog.detail.earning"
            :precision="2"
            :step="1"
            :controls="false"
            style="width: inherit"
        ...
        <el-form-item label="支出" prop="expense">
          <el-input-number
            v-model="updateDetailDialog.detail.expense"
            :precision="2"
            :step="1"
            :controls="false"
            style="width: inherit"
    ...
    >
      ...
      >
        <el-form-item label="日期时间" prop="date">
          <el-date-picker
            v-model="addDialog.detailForm.data.date"
            type="datetime"
            placeholder="选择日期时间"
        ...
        <el-form-item label="收入" prop="earning">
          <el-input-number
            v-model="addDialog.detailForm.data.earning"
            :precision="2"
            :step="1"
            :controls="false"
            style="width: inherit"
        ...
        <el-form-item label="支出" prop="expense">
          <el-input-number
            v-model="addDialog.detailForm.data.expense"
            :precision="2"
            :step="1"
            :controls="false"
            style="width: inherit"
      ...
      <upload
        :url="addDialog.pictureData.uploadUrl"
        ref="pictureUpload"
        list-type="picture"
        :limit-size="512"
        accept=".jpg, .jpeg, .png, .gif, .bmp, .JPG, .JPEG, .PNG, .GIF, .BMP"
        v-show="addDialog.step===1"
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button
            size="medium"
            @click="addDialog.step=0"
            v-show="addDialog.step===1&&!addDialog.justAddPicture"
            style="float:left;"
          >上一步</el-button>
          <el-button
            size="medium"
...
export default {
    ...
    return {
      ...
      addDialog: {
        ...
        rules: {
          ...
          department: [
            {
              validator: this.validateSelection,
              required: true,
              trigger: "change"
            }
          ],
  ...
  methods: {
    generateExpenseClaimForm() {
      ...
      excelApi
        .generateExpenseClaimForm(this.tableData.multipleSelectionData.data)
        .then(result => {
          window.open(
            this.baseUrl + `/excel/expenseClaimForm?formId=${result.data}`
          );
          ...
          if (!detail.id) {
            ...
            detailApi
              ...
              .then(result => {
                ...
                this.openAddPictureDialog(
                  this.addDialog.detailForm.data.id,
                  null
                );
              })
    ...
    onTableRowSelect(selection, row) {
      if (
        selection.length > this.tableData.multipleSelectionData.maxSelectedNum
      ) {
        this.$message.info({
        ...
        this.$confirm("此记录已生成过费用报销单, 是否继续?", "提示", {
          ...
          beforeClose: (action, instance, done) => {
            if (action === "confirm") {
              ...
              this.tableData.multipleSelectionData.data.splice(
                this.tableData.multipleSelectionData.data.indexOf(row)
              );
              this.$refs.table.toggleRowSelection(row, false);
              done();
            }
    ...
    showPicture(event, detailId, index) {
      ...
      if (
        detail.hasPicture &&
        event.length &&
        !(detail.pictures && detail.pictures.length > 0)
      ) {
        //有图片,没图片数据,且是打开折叠面板
    ...
    refreshPictureData() {
      if (this.addDialog.pictureData.index != null) {
        ...
        this.pictureCollapse.loading = true;
        detailApi
          .listPicture(this.addDialog.pictureData.detailId)
          .then(result => {
            //处理url
            result.data.forEach(i => {
              i.url = `${this.baseUrl}/detail/picture/img/${i.uri}`;
            ...
            this.tableData.data[
              this.addDialog.pictureData.index
            ].hasPicture = true;
            this.tableData.data[this.addDialog.pictureData.index].pictures =
              result.data; //搞不懂这里为啥是响应式
            this.pictureCollapse.loading = false;
          })
          .catch(() => (this.pictureCollapse.loading = false));
      } else {
        //新记录添加图片成功
        let tableDataIndex = this.handleTableDataIndex(
          this.addDialog.pictureData.index
        );
